...
def demo():
    ...
    w = 0.05
    out = blocked_loo_blend(p, {"c": cid}, ts, w=w, block_days=7)
    ...
    big = blocked_loo_blend(p, {"c": cid}, ts, w=w, block_days=3650)
    ...
    out_series = blocked_loo_blend(p, {"c": pd.Series(cid)}, pd.Series(ts), w=w)
    ...
    # a bare (non-dict) id array still works, and multi-entity averages the LOOs
    assert np.allclose(out, blocked_loo_blend(p, cid, ts, w=w, block_days=7))
    two = blocked_loo_blend(p, {"a": cid, "b": ["x"] * 6}, ts, w=w, block_days=7)
